(function($) {
    ...
    $.jnoty = function(m, o) {
        ...
        if ($('#jnoty').length === 0)
            $('<div id="jnoty"></div>')
            .addClass(
                (o && o.position) ? o.position : $.jnoty.defaults.position)
            .appendTo(
                (o && o.appendTo) ? o.appendTo : $.jnoty.defaults.appendTo);
        ...
        if ($.isFunction(this.each)) {
            ...
            return this.each(function() {
                ...
                if ($(this).data('jnoty.instance') === undefined) {
                    $(this)
                    .data('jnoty.instance', $.extend(new $.fn.jnoty(), {
                        notifications: [],
                        element: null,
                        interval: null
                    }));
                ...
                if ($.isFunction($(this).data('jnoty.instance')[m])) {
                    $(this).data('jnoty.instance')[m].apply(
                        $(this).data('jnoty.instance'),
    ...
    $.extend($.fn.jnoty.prototype, {
        ...
        render: function(n) {
            ...
            .addClass('jGrowl-notification alert ' + o.themeState +
                ' ui-corner-all' +
            ...
            .append($('<span/>')
            .addClass('jnoty-title')
            .html(o.header)))
            .append(
                $('<div/>').addClass('jnoty-message').html(message)))
            .data('jnoty', o)
            .addClass(o.theme)
            .children('.jnoty-close')
            ...
            $(notification).bind('mouseover.jnoty', function() {
                $('.jnoty-notification', self.element)
                .data('jnoty.pause', true);
            }).bind('mouseout.jnoty', function() {
                $('.jnoty-notification', self.element)
                .data('jnoty.pause', false);
                ...
                if (o.open.apply(notification,
                    [notification, message, o, self.element]) !== false) {
                    if (o.glue == 'after') {
                        $('.jnoty-container:last', self.element)
                        .after(notification);
                    }
                    else {
                        $('.jnoty-container:first', self.element)
                        .before(notification);
                    }
                    ...
                    .animate(o.animateOpen, o.openDuration, o.easing,
                        function() {
                            ...
                            if ($(this).data('jnoty') !== null &&
                                typeof $(this).data('jnoty') !==
                                'undefined') // Happens when a notification is closing before it's open.
                ...
                $(this)
                .animate(o.animateClose, o.closeDuration, o.easing,
                    function() {
            ...
            if (o.corners !== '' && $.fn.corner !== undefined) $(notification)
            .corner(o.corners);

            /** Add a Global Closer if more than one notification exists **/
            if ($('.jnoty-container:parent', self.element).length > 1 &&
                $('.jnoty-closer', self.element).length === 0 &&
                this.defaults.closer !== false) {
                $(this.defaults.closerTemplate)
                .addClass('jnoty-closer ' + this.defaults.themeState +
                    ' ui-corner-all')
                .addClass(this.defaults.theme)
                .appendTo(self.element)
                .animate(this.defaults.animateOpen, this.defaults.speed,
        ...
        update: function() {
            $(this.element).find('.jnoty-container:parent').each(function() {
                if ($(this).data('jnoty') !== undefined &&
                    $(this).data('jnoty').created !== undefined &&
                    ($(this).data('jnoty').created.getTime() +
                        parseInt($(this).data('jnoty').life, 10)) <
                    (new Date()).getTime() &&
                    $(this).data('jnoty').sticky !== true &&
                    ($(this).data('jnoty.pause') === undefined ||
                        $(this).data('jnoty.pause') !== true)) {
            ...
            if (this.notifications.length > 0 &&
                (this.defaults.pool === 0 ||
                    $(this.element).find('.jnoty-container:parent').length <
                    this.defaults.pool))
            ...
            if ($(this.element).find('.jnoty-container:parent').length < 2) {
                $(this.element)
                .find('.jnoty-closer')
                .animate(this.defaults.animateClose, this.defaults.speed,
        ...
        startup: function(e) {
            this.element = $(e)
            .addClass('jnoty')
            .append('<div class="jnoty-container"></div>');
